...
function _Images_ProcessSrc_CopyImageToHost( &$ctxProcess, $imgSrc, $imgCont, $settCache, $settImg )
{
	...
	if( !$type || !adkxsshiujqtfk( $ctxProcess, $settCache, array( 'img', $type ), $imgCont, $imgSrc -> src, $file ) )
		return( false );
...
function Images_ProcessSrcSizeAlternatives( $imgSzAlternatives, &$ctxProcess, $imgSrc, $settCache, $settImg, $settCdn, $isImportant = false )
{
	...
	foreach( array( $info[ 'cx' ], 			1920,	1366,	992,	768,	480,	360	 ) as $i => $cx )
	{
		...
		$imgScaled = null;
		foreach( array( $cx, 			1920,	1366,	992,	768,	480,	360	 ) as $cxCrop )
		{
			...
			$imgSzAlternatives -> a[ ( $info[ 'cx' ] != $cx ? ( string )$cx : 'O' ) . ( $cxCrop != $cx ? ( '@' . $cxCrop ) : '' ) ] = array( 'img' => $imgSrcAlter, 'sz' => array( $cx, $cy ) );
		}
...
function LazyLoad_SrcSubst( $info, $exact = false, $fill = null )
{
	if( !$info )
		$info = array();
...
function _Images_ProcessItemLazy_Finish( &$ctxProcess, $doc, $settImg, $item, $imgSrc )
{
	...
	$item -> setAttribute( 'src', LazyLoad_SrcSubst( $imgSrc ? $imgSrc -> GetInfo() : null, true ) );
...
function Images_ProcessItemLazyBg( &$ctxProcess, $doc, $settImg, $item, $imgSrc )
{
	...
	$imgSrc -> src = LazyLoad_SrcSubst( $imgSrc -> GetInfo(), true );
