...
class PluginRmtCfg
{
	...
	static function Update( $bForce = false, $bFirstTimeOnly = false )
	{
		...
		if( $lastCheckVer !== '2.20.33' || $lastCheckPackage !== 'Ext' )
		{
			$state = Plugin::StateGet();

			if( $lastCheckVer !== '2.20.33' && !isset( $state[ 'changeVerCheck' ] ) )
			{
		...
		if( !$bForce )
		{
			if( $bFirstTimeOnly && $lastCheckVer == '2.20.33' )
				return( Gen::S_FALSE );
...
class Plugin
{
		...
		if( $isAdminMode )
		{
			...
			add_filter( 'plugin_action_links_' . Plugin::GetCurBaseName(),
				function( $actions, $plugin_file )
				{
					...
					if( version_compare( Gen::GetArrField( $rmtCfg, 'Info.LastVersion', '2.20.33' ), '2.20.33' ) > 0 )
					{
	...
	static function GetAboutPluginContent()
	{
		...
		$urlAboutPluginImg = file_exists( __DIR__ . '/../Images/ProductLogo.png' ) ? add_query_arg( array( 'v' => '2.20.33' ), Plugin::FileUri( '../Images/ProductLogo.png', __FILE__ ) ) : null;
		...
		$url3rdPartySoft = file_exists( __DIR__ . '/../third-party-software.html' ) ? add_query_arg( array( 'v' => '2.20.33' ), Plugin::FileUri( '../third-party-software.html', __FILE__ ) ) : null;
	...
	static function GetAboutVendorContent()
	{
		...
		$urlAboutUsLogoImg = file_exists( __DIR__ . '/../Images/VendorLogo.png' ) ? add_query_arg( array( 'v' => '2.20.33' ), Plugin::FileUri( '../Images/VendorLogo.png', __FILE__ ) ) : null;
	...
	static private function _PrevVer_Check()
	{
		...
		if( empty( $warningVersionInfo ) )
		{
			...
			$verTo = self::_PrevVer_GetInt( '2.20.33' );
			if( $verTo < $verFrom )
...
?>
	...
	static private function _TransientUpdatePlugins_Process( &$value, $plgVer, $switch )
	{
		...
		if( $switch )
			$dwnldUrl = Gen::GetFileName( $dwnldUrl, true, true ) . '.2.20.33.' . Gen::GetFileExt( $dwnldUrl );
		else
		{
		...
		if( ( $switch || version_compare( $verLast, '2.20.33' ) > 0 ) && !isset( $value -> response[ Plugin::GetCurBaseName() ] ) )
		{
		...
		if( isset( $value -> response[ Plugin::GetCurBaseName() ] ) )
		{
			...
			if( !$switch && version_compare( $verLast, '2.20.33' ) > 0 )
	...
	static function CmnStyle( $ids )
	{
		...
		foreach( $ids as $id )
		{
			...
			wp_enqueue_style( Plugin::CmnScriptId( $id ), add_query_arg( Plugin::GetFileUrlPackageParams(), $fileUrl . '/' . $id . '.css' ), array(), '2.20.33' );
	...
	static function CmnScripts( $ids )
	{
		...
		foreach( $ids as $id )
		{
			...
			wp_register_script( $scrHndId, add_query_arg( Plugin::GetFileUrlPackageParams(), $fileUrl . '/' . $id . '.js' ), $deps, '2.20.33' );
			if( $id == 'Gen' )
				...
				<script>
					(function()
					{
						...
						function _ProcessAnswerAndDeactivateAndClose( answerId, answerUserData )
						{
							...
							sendDataUrl = sendDataUrl.replace( "{PluginVersion}",				encodeURI( "2.20.33" ) );
