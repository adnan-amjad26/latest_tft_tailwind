...
use seraph_accel\Sabberworm\CSS\Parsing\OutputException;
...
use seraph_accel\Sabberworm\CSS\Value\Size;
use seraph_accel\Sabberworm\CSS\Value\Color;
...
class DeclarationBlock extends RuleSet {
	...
	public static function parse(ParserState $oParserState) {
		$oResult = new DeclarationBlock($oParserState->currentPos());
		$oResult->parseSelectors($oParserState);
		RuleSet::parseRuleSet($oParserState, $oResult);
		return $oResult;
	}

	public function parseSelectors(ParserState $oParserState) {
		$aComments = array();
		$iStartPos = $oParserState->currentPos();

		{
			...
			for( ;; )
			{
				$selector .= $oParserState->consumeUntil($nBracket > 0 ? '\\(\\)\\{\\}\\;\\@' : '\\,\\(\\)\\{\\}\\;\\@', false, false, true, $aComments);
				$sSym = $oParserState->peek();

				if( $sSym == ',' )
				{
					$oParserState->consume(1, false);
					$this->aSelectors[] = new Selector($selector, false, $iPos);
					$selector = '';
				...
				if( $sSym == ')' )
				{
					...
					if( $nBracket < 0 )
					{
						$oParserState->traceException(new SourceException(Settings::ParseErrMed, "Unopened bracket in selector", $oParserState->currentPos()));
						$nBracket = 0;
					}
				...
				if( $nBracket > 0 )
					$oParserState->traceException(new SourceException(Settings::ParseErrMed, 'Unclosed ' . $nBracket . ' bracket(s) in selector', $iStartPos));
		...
		if(!$oParserState->comes('{')) {
			if($oParserState->comes(';'))
				$oParserState->consume(1, false);
			throw new SourceException(Settings::ParseErrMed, "Unexpected end of selector", $oParserState->currentPos());
		}
		$oParserState->consume(1, false);
	...
	public function expandBorderShorthand() {
		...
		foreach ($aBorderRules as $sBorderRule) {
			...
			foreach ($aValues as $mValue) {
				...
				$oNewRule = new Rule($sNewRuleName, $this->iPos);
	...
	public function expandDimensionsShorthand() {
		...
		foreach ($aExpansions as $sProperty => $sExpanded) {
			...
			foreach (array('top', 'right', 'bottom', 'left') as $sPosition) {
				$oNewRule = new Rule(sprintf($sExpanded, $sPosition), $this->iPos);
				$oNewRule->setIsImportant($oRule->getIsImportant());
	...
	public function expandFontShorthand() {
		...
		foreach ($aFontProperties as $sProperty => $mValue) {
			$oNewRule = new Rule($sProperty, $this->iPos);
	...
	public function expandBackgroundShorthand() {
		...
		$aBgProperties = array(
			...
			'background-position' => array(new Size(0, '%', null, false, $this->iPos), new Size(0, '%', null, false, $this->iPos))
		);
		$mRuleValue = $oRule->getValue();
		$aValues = array();
		if (!$mRuleValue instanceof RuleValueList) {
	...
	public function createShorthandProperties(array $aProperties, $sShorthand) {
		...
		if (count($aNewValues)) {
			$oNewRule = new Rule($sShorthand, $this->iPos);
	...
	public function createFontShorthand() {
		...
		$oNewRule = new Rule('font', $this->iPos);
		...
		if (isset($aRules['line-height'])) {
			...
			if ($aLHValues[0] !== 'normal') {
				$val = new RuleValueList('/', $this->iPos);
		...
		$oFFValue = new RuleValueList(',', $this->iPos);
	...
	public function render(string &$sResult, \seraph_accel\Sabberworm\CSS\OutputFormat $oOutputFormat) {
		...
		//    throw new OutputException("Attempt to print declaration block with missing selector", $this->iPos/* MBI!!!! Must be LINE NO */);
		//}
		...
		$oOutputFormat->getFormatter()->implode($sResult, $oOutputFormat->spaceBeforeSelectorSeparator() . ',' . $oOutputFormat->spaceAfterSelectorSeparator(), $this->aSelectors);
