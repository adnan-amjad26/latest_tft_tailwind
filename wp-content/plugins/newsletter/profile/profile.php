...
class NewsletterProfile extends NewsletterModule {
    ...
    function get_profile_form($user) {
        ...
        if (!empty($options['name'])) {
            ...
            $buffer .= '<input class="tnp-firstname" type="text" name="nn" value="' . esc_attr($user->name) . '"' . ($form['name_rules'] == 1 ? ' required' : '') . '>';
            $buffer .= "</div>\n";
        }
        ...
        if (!empty($options['surname'])) {
            ...
            $buffer .= '<input class="tnp-lastname" type="text" name="ns" value="' . esc_attr($user->surname) . '"' . ($form['surname_rules'] == 1 ? ' required' : '') . '>';
            $buffer .= "</div>\n";
        }
