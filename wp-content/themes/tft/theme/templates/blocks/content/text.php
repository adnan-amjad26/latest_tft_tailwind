...
if (isset($fields['backgrounds'])) {
	...
	switch ($fields['backgrounds']['dark_theme']) {
		case 'indigo':
			$dark_bottom_overlay = 'dark:bg-[linear-gradient(0deg,_rgba(31,30,44,0.00)_0%,_#1F1E2C_65%)]';

			break;
		case 'indigo-dark':
			$dark_bottom_overlay = 'dark:bg-[linear-gradient(0deg,_rgba(26,25,38,0.00)_0%,_#1A1926_65%)]';
...
?>
<section
		class="z-30 relative pb-[100px] overflow-visible text-white font-body font-medium leading-tight text-[16px] xl:text-[24px] xl:pb-[200px] <?= $table_classes; ?> <?= $td_classes; ?> <?= $th_classes; ?> <?= $link_classes; ?> <?= $list_classes; ?> <?= $strong_classes; ?> <?= $p_classes; ?> <?= $h2_classes; ?> <?= $h3_classes; ?> <?= $ol_classes; ?>">
	<div class="max-w-[1920px] mx-auto px-[10px] text-center md:text-left md:px-[30px] lg:px-[50px] xl:px-[110px]">
		<?= $fields['text']; ?>
	</div>
	<div class="absolute w-full top-[calc(100%-100px)] h-[300px] <?= $bottom_overlay; ?> <?= $dark_bottom_overlay; ?>">
	</div>
</section>
