jQuery(document).ready(function($) {
	...
	$('.btn-menu').click(function() {
		$('header').toggleClass('overlay').find('.menu-nav-wrap').toggleClass('active');
	});
	$('.btn-menu-close').click(function() {
		$('header').removeClass('overlay').find('.menu-nav-wrap').removeClass('active');
	});
    ...
    $('.mode .toggle-input').click(function() {
        ...
        (typeof day !== 'undefined' && day !== false) ? $('body').removeAttr('data-day') : $('body').attr('data-day', '');
        // Update Scroll for "is-inview"
        scroll.update();
        // localStorage set mode
        (localStorage.getItem('dnmode') == 'night') ? localStorage.setItem('dnmode', 'day') : localStorage.setItem('dnmode', 'night');
    });
    ...
    if(hash) $('html, body').animate({scrollTop:$(hash).offset().top - 100}, 500);
    ...
    $('.content-scrolls .toggles li').click(function() {
        ...
        $('.content-scrolls .scrolls-box[data-type="'+$(this).data('type')+'"]').show();
    });

    // Home wrap bricks element
    if($('body').hasClass('home')) $('.home .bricks-elem').wrapAll('<div class="bricks"/>');

    // Comparison Firms Tab
    $('.comparison .toggles li').click(function() {
        $(this).addClass('active').siblings().removeClass('active');
        $('.comparison .comparison-firm').removeClass('active');
        $('.comparison .comparison-firm[data-firm="'+$(this).data('firm')+'"]').addClass('active');
    });
	...
	var comparisonTableMobileSwiper = new Swiper('.comparison-table-mobile-swiper', {
		navigation: {
    ...
    $(window).on('load', function() {
        // Challenge Type
        var challenge_type = $('.toggles li[challenge_type].active').attr('challenge_type');
        $('.toggles li[account_type]').each(function() {
            var account_type = $(this).attr('account_type');
            if(!$('.product-table[challenge_type='+challenge_type+'][account_type='+account_type+']').length) $(this).hide();
        });
        // Account Type
        var account_type = $('.toggles li[account_type]:visible').first().addClass('active').attr('account_type');
        $('.toggles li[account_size]').each(function() {
            var account_size = $(this).attr('account_size');
            if(!$('.product-table[challenge_type='+challenge_type+'][account_type='+account_type+'][account_size='+account_size+']').length) $(this).hide();
        });
        // Account Size
        var account_size = $('.toggles li[account_size]:visible').first().addClass('active').attr('account_size');
        // Product Table
        $('.product-table[challenge_type='+challenge_type+'][account_type='+account_type+'][account_size='+account_size+']').addClass('active');
    });
    ...
    $('.toggles li[challenge_type]').click(function() {		
        ...
        $('.toggles li[account_type]').each(function() {
            var account_type = $(this).attr('account_type');
            ($('.product-table[challenge_type='+challenge_type+'][account_type='+account_type+']').length) ? $(this).show() : $(this).hide();
        });
        ...
        $('.toggles li[account_size]').each(function() {
            var account_size = $(this).attr('account_size');
            ($('.product-table[challenge_type='+challenge_type+'][account_type='+account_type+'][account_size='+account_size+']').length) ? $(this).show() : $(this).hide();
        });
        $('.toggles li[account_size]').removeClass('active');
        var account_size = $('.toggles li[account_size]:visible').first().addClass('active').attr('account_size');
        // Product Table
        $('.product-table[challenge_type='+challenge_type+'][account_type='+account_type+'][account_size='+account_size+']').addClass('active').siblings().removeClass('active');
		...
		if($(window).width() < 992) {
			for(let i = 0; i < productTableMobileSwiper.length; i++) productTableMobileSwiper[i].slideTo(0);
			for(let i = 0; i < comparisonTableMobileSwiper.length; i++) comparisonTableMobileSwiper[i].slideTo(0);
		}
    });
    ...
    $('.toggles li[account_type]').click(function() {
        ...
        // Account Type
        $(this).addClass('active').siblings().removeClass('active');
        var account_type = $('.toggles li[account_type].active').attr('account_type');
        // Account Size
        $('.toggles li[account_size]').each(function() {
            var account_size = $(this).attr('account_size');
            ($('.product-table[challenge_type='+challenge_type+'][account_type='+account_type+'][account_size='+account_size+']').length) ? $(this).show() : $(this).hide();
    ...
    $('.toggles li[account_size]').click(function() {
        ...
        // Account Size
        $(this).addClass('active').siblings().removeClass('active');
        var account_size = $('.toggles li[account_size].active').attr('account_size');
        // Product Table
        $('.product-table[challenge_type='+challenge_type+'][account_type='+account_type+'][account_size='+account_size+']').addClass('active').siblings().removeClass('active');
		// Disable Quick Comparison
		$('.table__tab[quick_comparison]').removeClass('active');
    ...
    $(window).on('load', function() {
        var challenge_type = $('#order_review [challenge_type]').attr('challenge_type');
        ...
        $('form[name=checkout] input[value='+challenge_type+']').prop('checked', true);
        $('form[name=checkout] input[value='+account_size+']').prop('checked', true);
        $('form[name=checkout] input[value='+account_type+']').prop('checked', true);
        $('form[name=checkout] input[value='+platform+']').prop('checked', true);
        $('form[name=checkout] input[value='+broker+']').prop('checked', true);
        $('.chooseChallengeTable > div[challenge_type='+challenge_type+'][account_type='+account_type+'][account_size='+account_size+']').addClass('active');
        $('.checkout-toggles [account_type]').each(function() {
            var account_type = $(this).attr('account_type');
            if(!$('.chooseChallengeTable > div[challenge_type='+challenge_type+'][account_type='+account_type+']').length) $(this).hide();
        });
        $('.checkout-toggles [account_size]').each(function() {
            var account_size = $(this).attr('account_size');
            if(!$('.chooseChallengeTable > div[challenge_type='+challenge_type+'][account_type='+account_type+'][account_size='+account_size+']').length) $(this).hide();
        });
        if(challenge_type == 'knights-challenge' || platform == 'mt5') $('[broker=purple-trading]').hide();
    });
    $('body').on('change', 'input[name=challenge_type], input[name=account_size], input[name=account_type], input[name=platform], input[name=broker]', function() {
        var challenge_type = $('.checkout-toggles [challenge_type] input:checked').val();
        var platform = $('.checkout-toggles [platform] input:checked').val();
        if($(this).is('[name=challenge_type]')) {
            var challenge_type = $('.checkout-toggles [challenge_type] input:checked').val();
            $('.checkout-toggles [account_type]').each(function() {
                var account_type = $(this).attr('account_type');
                if($('.chooseChallengeTable > div[challenge_type='+challenge_type+'][account_type='+account_type+']').length) $(this).show();
                else $(this).hide();
            });
            $('.checkout-toggles [account_type]:visible:first').find('input').prop('checked', true);
            var account_type = $('.checkout-toggles [account_type] input:checked').val();
            $('.checkout-toggles [account_size]').each(function() {
                var account_size = $(this).attr('account_size');
                if($('.chooseChallengeTable > div[challenge_type='+challenge_type+'][account_type='+account_type+'][account_size='+account_size+']').length) $(this).show();
                else $(this).hide();
            });
            $('.checkout-toggles [account_size]:visible:first').find('input').prop('checked', true);
            var account_size = $('.checkout-toggles [account_size] input:checked').val();
            $('.checkout-toggles [broker]:visible:first').find('input').prop('checked', true);
            (challenge_type == 'knights-challenge' || platform == 'mt5') ? $('[broker=purple-trading]').hide() : $('[broker=purple-trading]').show();
...
// 	updateTextFill();
	...
	$('.product-table-mobile [data-spoiler]').click(function() {
		...
		$(this).closest('.product-table-mobile').find('.product-table-mobile-swiper .table__body > .row:nth-child('+index+')').next().height(height).toggle();
	});
	
	// Brokers
	$('button[data-broker]').click(function() {
		$('.button-group[data-broker]').removeClass('active');
		$('.button-group[data-broker="'+$(this).data('broker')+'"]').addClass('active');
	});
	...
	jQuery('.table__expand').each(function() {
		...
		jQuery(this).find('.col-xxs-3').each(function(i) {
	...
	$('.table__tab[quick_comparison]').click(function() {
		...
		var comparison_table = $('.comparison-table[challenge_type="'+table.attr('challenge_type')+'"]');
		comparison_table.toggle();
		table.find('.table:not(.comparison-table)').toggle().after(comparison_table);
	});
