					...
					<div class="title relative">
						...
						<div class="torch absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
							<img decoding="async" loading="lazy"
								 src="<?php echo get_template_directory_uri(); ?>/img/torch.svg" alt="torch">
							<div class="glow"></div>
						</div>
					</div>
				</div>
			...
			<div class="row justify-center mt-12 lg:mt-24">
					...
					<?php if ($challenge_type): ?>
						<ul class="toggles w-full first row gap-6 lg:gap-12 justify-center font-headers text-lg ">
							<?php foreach ($challenge_type as $attr): ?>
								...
								<li <?php if ($image['url']) echo ' style="background-image: url(' . $image['url'] . ');"'; ?>
									challenge_type="<?php echo $attr->slug; ?>" <?php if ($challenge_type[0]->slug == $attr->slug) {
									echo ' class="challenge_type active"';
								} else {
									echo 'class = "challenge_type" ';
								} ?>>
					...
					<?php if ($account_type): ?>
						<ul class="toggles w-full second row gap-6 lg:gap-12 justify-center font-headers text-lg mt-12 lg:mt-16">
							<?php foreach ($account_type as $attr): ?>
								<?php $image = get_field('image', $attr); ?>
								<li class="account_type" <?php if ($image['url']) echo ' style="background-image: url(' . $image['url'] . ');"'; ?>
									account_type="<?php echo $attr->slug; ?>">
									<span><?php echo $attr->name; ?></span>
								</li>
							<?php endforeach; ?>
						</ul>
					...
					<?php if ($account_size): ?>
						<ul class="toggles w-full third row gap-6 lg:gap-12 justify-center font-headers text-lg mt-12 lg:mt-16">
							<?php foreach ($account_size as $attr): ?>
								<li account_size="<?php echo $attr->slug; ?>" class="account_size">
									<span><?php echo $attr->name; ?></span>
								</li>
							<?php endforeach; ?>
						</ul>
				...
				<?php if ($tables): ?>
					...
					foreach ($tables as $table): ?>
						...
						<div class="product-table" challenge_type="<?php echo $challange_type; ?>"
							 account_type="<?php echo $account_type; ?>" account_size="<?php echo $account_size; ?>">
							<?php $table = $table['table']; ?>
							<?php if ($quick_comparison): ?>
								<div class="table__header">
									<div class="table__tab"
										 quick_comparison><?php _e('Quick Comparison', 'cbd'); ?></div>
								</div>
							<?php endif; ?>
													...
													<div class="table__body">
														...
														foreach ($table['body'] as $row): ?>
															...
															<?php foreach ($row as $cell): ?>
																<div
																	class="col-xxs-3<?php if (!$cell['c']) echo ' empty'; ?>"><?php echo $cell['c']; ?></div>
															<?php endforeach; ?>
							...
							<div class="table__body">
								<?php foreach ($table['body'] as $row): ?>
									<div class="row">
										<?php foreach ($row as $cell): ?>
											<div
												style="min-width: <?php echo 100 / count($row); ?>%;"><?php echo $cell['c']; ?></div>
										<?php endforeach; ?>
									</div>
								<?php endforeach; ?>
							</div>
