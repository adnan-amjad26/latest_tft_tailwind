					...
					<div class="classChoicesTopDiv">
						<?php if ($challenge_type) : ?>
							<div class="classChoiceTopButtons">
								<?php $i = 1;
								foreach ($challenge_type as $attr) : ?>
								...
								<?php if ($body) : ?>
									<?php foreach ($body as $row) : ?>
										<div class="chooseChallengeRowDiv">
											...
											foreach ($row as $c) : ?>
												<?php if (strlen($c['c'])) : ?>
													<div class="<?php if ($j > 0 && !strlen($row[1]['c']) && !strlen($row[3]['c'])) echo 'chooseChallengeColumnBalanceDiv';
																elseif ($j > 0 && !strlen($row[2]['c'])) echo 'chooseChallengeColumnBalance2Div';
																else echo 'chooseChallengeColumnDiv'; ?> <?php echo ($j == 0) ? 'chooseChallengeColumn1' : 'chooseChallengeColumn234'; ?><?php if ($j == 2 && strlen($row[1]['c']) && strlen($row[3]['c'])) echo ' chooseChallengeColumn2nd'; ?><?php if ($j >= 3 && strlen($row[1]['c']) && strlen($row[3]['c'])) echo ' chooseChallengeColumn3rd'; ?>"><?php echo $c['c']; ?></div>
												<?php endif; ?>
											<?php $j++;
											endforeach; ?>
										</div>
